...
graph TB
    ...
    B --> E["公式:<br/>P(A) = 事件发生次数/总实验次数"]
    B --> F["例子:<br/>图钉实验<br/>扔1000次,300次针尖朝上<br/>P ≈ 30%"]
    B --> G["依赖:<br/>大数定律<br/>次数越多越准确"]

    C --> H["前提:<br/>等可能性<br/>结果机会均等"]
    C --> I["公式:<br/>P(A) = 有利结果数/总可能结果数"]
    C --> J["例子:<br/>掷骰子<br/>P(6) = 1/6<br/>不需实验"]
...
graph TB
    ...
    B -->|计算"不是A"| C["补集法则<br/>P(A') = 1 - P(A)"]
    C --> C1["关键词:<br/>'至少一个'<br/>'不是全部'"]
    C --> C2["例子:<br/>至少有一天下雨<br/>= 1 - P(365天都不下雨)"]

    B -->|计算"A或B"| D{A和B能<br/>同时发生吗?}
    D -->|不能<br/>互斥| E["加法法则(互斥)<br/>P(A or B) = P(A) + P(B)"]
    D -->|可以<br/>不互斥| F["加法法则(一般)<br/>P(A or B) = P(A) + P(B) - P(A and B)"]

    E --> E1["例子:<br/>掷骰子<br/>P(1或6) = 1/6 + 1/6"]
    F --> F1["例子:<br/>扑克牌<br/>P(红心或K)<br/>要减去红心K"]

    B -->|计算"A且B"| G{A和B<br/>相互独立吗?}
    G -->|独立| H["乘法法则(独立)<br/>P(A and B) = P(A) × P(B)"]
    G -->|不独立| I["乘法法则(条件)<br/>P(A and B) = P(A) × P(B|A)"]

    H --> H1["例子:<br/>两次抛硬币<br/>P(都是正面)<br/>= 1/2 × 1/2"]
    I --> I1["例子:<br/>不放回抽牌<br/>P(两张都是K)<br/>= 4/52 × 3/51"]
...
graph TB
    ...
    A[初始状态<br/>P(嫌疑人A有罪)] --> B["新证据出现<br/>例如:发现指纹"]

    B --> C[更新判断<br/>计算条件概率]

    C --> D["P(A有罪|发现指纹)<br/>= P(有罪且有指纹) / P(发现指纹)"]

    D --> E{概率是否<br/>发生变化?}

    E -->|是<br/>P(B|A) ≠ P(B)| F[事件相关<br/>A和B不独立]
    E -->|否<br/>P(B|A) = P(B)| G[事件独立<br/>A和B互不影响]
    ...
    L -->|是| M["不独立<br/>需要条件概率<br/>P(指纹|前科)"]
    L -->|否| N["独立<br/>P(指纹|前科) = P(指纹)"]
...
graph TB
    ...
    B -->|否<br/>单一选择| D{顺序<br/>重要吗?}

    D -->|是<br/>顺序重要| E["排列 Permutation<br/>P(n,k) or nPk"]
    D -->|否<br/>顺序不重要| F["组合 Combination<br/>C(n,k) or nCk"]

    E --> E1["记忆:<br/>P = Position<br/>位置重要"]
    E --> E2["例子:<br/>10人选3人<br/>担任金银铜牌<br/>(A,B,C) ≠ (C,B,A)"]
    E --> E3["公式:<br/>P(n,k) = n!/(n-k)!"]

    F --> F1["记忆:<br/>C = Committee<br/>委员会不分位置"]
    F --> F2["例子:<br/>10人选3人<br/>组成清洁小组<br/>(A,B,C) = (C,B,A)"]
    F --> F3["公式:<br/>C(n,k) = n!/[k!(n-k)!]"]

    G{如何快速判断?} --> H["关键问题:<br/>'ABC'和'CBA'<br/>是同一个结果吗?"]
